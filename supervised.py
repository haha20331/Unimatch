...
def evaluate(model, loader, mode, cfg):
    ...
    dice_score =  2*intersection_meter.sum / (union_meter.sum + 1e-10 +intersection_meter.sum) * 100.0
    mIOU = np.mean(iou_class)

    with open('good+normal_6labeled_IoU_score_log.txt', 'a') as f:
        f.write(str(iou_class)+'\n')
    with open('good+normal_6labeled_dice_score_log.txt', 'a') as f:
        f.write(str(dice_score)+'\n')
